/**
 * Aprobar / convertir lead — mismo camino que POST /api/crm/leads/[id]/approve.
 * Crea cuenta, contacto, negocio, instalaciones y cotización CPQ.
 */
...
export async function approveLead(opts: {
  tenantId: string;
  userId: string;
  leadId: string;
  ...
  try {
    ...
    const result = await prisma.$transaction(
      ...
      if (useExistingAccountId) {
        const existing = await tx.crmAccount.findFirst({
          where: { id: useExistingAccountId, tenantId: tenantId },
        });
        if (!existing) {
      ...
      if (contactResolution === "use_existing" && contactIdForResolution) {
        const existing = await tx.crmContact.findFirst({
          where: { id: contactIdForResolution, tenantId: tenantId },
        });
      ...
      if (lead.source === "email_forward") {
        const leadFileLinks = await tx.documentoEnlace.findMany({
          where: { tenantId: tenantId, entityType: "lead", entityId: lead.id },
          select: { fileId: true },
        });
      ...
      if (bodyProposalTemplateSlug) {
        const tpl = await tx.cpqProposalTemplate.findFirst({
          where: { slug: bodyProposalTemplateSlug, active: true, OR: [{ tenantId: tenantId }, { tenantId: null }] },
          select: { id: true },
        });
      ...
      if (!resolvedTemplateId) {
        ...
        if (industryType) {
          ...
          if (templateSlug) {
            const tpl = await tx.cpqProposalTemplate.findFirst({
              where: { slug: templateSlug, active: true, OR: [{ tenantId: tenantId }, { tenantId: null }] },
              select: { id: true },
            });
            if (tpl) resolvedTemplateId = tpl.id;
          }
      ...
      const tenantOrShared = { OR: [{ tenantId: tenantId }, { tenantId: null }] };
      ...
      for (const inst of installationsPayload) {
        ...
        if (dotacion.length > 0) {
          ...
          const quoteCode = await nextDocumentCode(tx, tenantId, "quote");
          ...
          if (instConditions.proposalTemplateId) {
            const tpl = await tx.cpqProposalTemplate.findFirst({
              where: { slug: instConditions.proposalTemplateId, active: true, OR: [{ tenantId: tenantId }, { tenantId: null }] },
              select: { id: true },
            });
          ...
          try {
            ...
            try {
              await applyDefaultQuoteIncludes(tx, quote.id, tenantId);
            } catch (includesErr) {
            ...
            if (leadCostItems.length > 0) {
              for (const ci of leadCostItems) {
                ...
                const catItem = await tx.cpqCatalogItem.findFirst({
                  where: { id: catId, active: true, OR: [{ tenantId: tenantId }, { tenantId: null }] },
                ...
                if (existingPuesto) {
                  ...
                  if (fallbackPuesto) {
                    ...
                    const createdPuesto = await tx.cpqPuestoTrabajo.create({
                      data: { tenantId: tenantId, name: fallbackPuestoName, active: true },
                      select: { id: true, name: true },
                    });
      ...
      if (!anyQuoteCreated) {
        ...
        try {
          ...
          try {
            await applyDefaultQuoteIncludes(tx, fallbackQuote.id, tenantId);
          } catch (includesErr) {
    ...
    import("@/modules/cpq/descriptions/generate-descriptions").then(({ generateQuoteDescription }) => {
      for (const q of result.quotes) {
        generateQuoteDescription(q.id, { tenantId: tenantId, userId: userId }).catch((err) =>
