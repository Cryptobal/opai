/**
 * Rechazar lead — mismo camino que POST /api/crm/leads/[id]/reject.
 */
...
export async function rejectLead(opts: {
  tenantId: string;
  userId: string;
  leadId: string;
  body: {
    reason: "spot_service" | "out_of_scope" | "no_budget" | "duplicate" | "no_response" | "other";
    note?: string | null;
    sendEmail?: boolean;
  ...
  try {
    ...
    if (body.sendEmail) {
      ...
      const signature = await resolveSignatureForSend(tenantId, userId);
      if (signature) {
