...
export function flowV3Error(error: unknown, fallback = "Error interno"): NextResponse {
  ...
  const known =
    /no encontrada|no encontrado|requerido|inválid|archivada|no puede|no pertenecen|no pertenece|no está vinculada|vinculad|programación|cuota|hito|lunes ISO|misma cuenta|admiten|Nada que actualizar|no hay nada que guardar|subfila|solo un nivel|categoría padre/i.test(
      message,
    );
