...
/**
 * Lápida CANCELLED en el ORIGEN tras mover una proyección pura cruzando de
 * bucket. Sin ella, el slot-claim del motor (que matchea por
 * occurrenceBucketKey(scheduledDate, recurrence)) libera el bucket original al
 * mover la cuota → la proyección de ese bucket se regenera (doble conteo). La
 * lápida (status CANCELLED, amountClp 0) ocupa la unique [itemId, scheduledDate]
 * y el motor la silencia (`if (mat?.status === "CANCELLED") continue;`). El
 * modelo no tiene isAutoGenerated, así que sólo se setean los campos requeridos.
 */
async function createMoveTombstone(
  db: typeof prisma,
  args: { tenantId: string; itemId: string; scheduledDate: Date },
): Promise<void> {
...
async function resolveCollisionAndMove(args: {
  ...
  // Datos del origen ANTES del update (que reescribe scheduledDate): sirven para
  // decidir si hay que dejar lápida al cruzar de bucket (ver más abajo).
  const sourceForTombstone =
    await prisma.financeCashflowOccurrence.findUnique({
      where: { id: occurrenceId },
      select: {
        scheduledDate: true,
        dteId: true,
        bankTransactionId: true,
        item: { select: { recurrence: true } },
      },
    });
  ...
  // Lápida en el origen al CRUZAR de bucket con una proyección pura. El camino
  // inverso (mover HACIA una fecha con lápida) ya lo cubre isCancelledTombstone
  // arriba: borra la lápida antes del update, así que volver nunca choca.
  const originalDate = sourceForTombstone?.scheduledDate;
  if (originalDate) {
    const recurrence = sourceForTombstone?.item?.recurrence ?? "MONTHLY";
    const isPureProjection =
      sourceForTombstone?.dteId == null &&
      sourceForTombstone?.bankTransactionId == null;
    const crossBucket =
      occurrenceBucketKey(originalDate, recurrence) !==
      occurrenceBucketKey(target, recurrence);
    if (isPureProjection && crossBucket) {
      await createMoveTombstone(prisma, {
        tenantId,
        itemId,
