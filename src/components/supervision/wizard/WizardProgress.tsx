...
import { MapPin, Users, ClipboardCheck, Camera, FileText, Check, AlertTriangle } from "lucide-react";
import type { WizardStep } from "./types";

const STEPS = [
  { step: 1 as WizardStep, label: "Check-in", icon: MapPin },
  { step: 2 as WizardStep, label: "Evaluacion", icon: Users },
  { step: 3 as WizardStep, label: "Verificacion", icon: ClipboardCheck },
...
type Props = {
  ...
  stepAlerts?: Record<number, boolean>;
};

export function WizardProgress({ currentStep, onStepClick, maxReachedStep, stepAlerts = {} }: Props) {
  return (
    <div className="flex items-center justify-between gap-1 px-2">
      {STEPS.map(({ step, label, icon: Icon }) => {
        ...
        const hasAlert = stepAlerts[step] && isCompleted;
