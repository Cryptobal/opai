...
export function PlatformAiProvidersConfig() {
  ...
  const fetchProviders = useCallback(async () => {
    try {
      const res = await fetch('/api/platform/ai-providers');
  ...
  const handleSync = async () => {
    setSyncing(true);
    try {
      const res = await fetch('/api/platform/ai-providers/sync', { method: 'POST' });
      const json = await res.json();
      if (json.success) {
  ...
  const handleActivate = async (providerId: string) => {
    ...
    try {
      const res = await fetch(`/api/platform/ai-providers/${providerId}`, {
        method: 'PUT',
...
function ProviderConfigDialog({
  ...
  const handleRevealKey = async () => {
    ...
    try {
      const res = await fetch(`/api/platform/ai-providers/${provider.id}/key`);
      const json = await res.json();
      if (json.success) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/platform/ai-providers/${provider.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (selectedModelId) {
        await fetch(`/api/platform/ai-providers/${selectedModelId}/set-default-model`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleTest = async () => {
    ...
    try {
      const res = await fetch(`/api/platform/ai-providers/${provider.id}/test`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
