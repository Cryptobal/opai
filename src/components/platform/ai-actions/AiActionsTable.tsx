...
function ErrorCell({ message }: { message: string | null }) {
  ...
  return (
    <div className="min-w-0">
      <span className="text-ds-body text-status-danger-fg">{short}</span>
      {message.length > 60 && (
        <button
          type="button"
...
const columns: DataTableColumn<AiActionRow>[] = [
  {
    id: "date",
    header: "Fecha",
    cell: (r) => <span className="text-ds-body text-ds-text-2">{formatDate(r.createdAt)}</span>,
  },
  {
    id: "tenant",
    header: "Tenant",
    cell: (r) => (
      <div className="min-w-0">
        <p className="truncate text-ds-body font-medium text-ds-text-1">{r.tenantName}</p>
  ...
  },
  {
    ...
    header: "Usuario",
    hideOnMobile: true,
    cell: (r) => <span className="text-ds-body text-ds-text-2">{r.userName}</span>,
  },
  {
    id: "tool",
    header: "Tool",
  ...
  },
  {
    ...
    align: "right",
    hideOnMobile: true,
    cell: (r) => <span className="ds-num text-ds-body text-ds-text-2">{r.durationMs}</span>,
  },
  {
    id: "entity",
    header: "Entidad",
    cell: (r) => {
      ...
      return (
        <Link href={path} className="text-ds-body text-primary hover:underline">
        ...
        {rows.map((r) => (
          <li key={r.id}>
            <Surface elevation={1} padding="sm" className="space-y-1">
              ...
              <p className="font-medium text-ds-body text-ds-text-1">{r.toolName}</p>
