 ...
 *
 * Móvil (< sm): lista vertical cronológica con los próximos 4 eventos
 * (hora · título · contexto · estado), sin mini-columnas de días.
 * Desktop (≥ sm): bloque "Hoy" + grilla de próximos días (comportamiento
 * ejecutivo previo).
 */
export function AgendaHubCard() {
  ...
  return (
    <Surface elevation={1} padding="md" className="min-w-0 space-y-3">
      ...
      {loading ? (
        // Altura estable para evitar layout shift al llegar los datos.
        ...
        <>
          {/* ── Móvil: lista vertical cronológica (máx 4 próximos) ── */}
          <AgendaHubUpcomingList items={items} className="sm:hidden" />

          {/* ── Desktop: bloque Hoy + grilla de próximos días ── */}
                ...
                <ul className="space-y-1">
                  {todayItems.map((i) =>
                    i.source === "tarea" ? (
                      ...
                      >
                        <span className={`min-w-0 truncate ${i.allDay ? "text-tint-violet-fg" : "text-ds-text-1"}`}>
