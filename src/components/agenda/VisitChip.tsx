...
export function VisitChip({ type, title, start, syncStatus, onClick }: Props) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`w-full rounded-xl border px-2 py-1.5 text-left text-[12px] ds-tap ${TYPE_CLASS[type] ?? TYPE_CLASS.otra}`}
    >
