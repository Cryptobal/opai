...
export function RondasMonitoreoClient({
  ...
  const guardPanelData = useMemo(() => {
    return filtered.map((r: any) => ({
      ...
      installationId: r.rondaTemplate?.installation?.id ?? r.installationId ?? null,
      templateName: r.isAdHoc ? "Ronda Libre" : (r.rondaTemplate?.name ?? "Ronda"),
      isAdHoc: r.isAdHoc ?? false,
      installationName: r.rondaTemplate?.installation?.name ?? r.installation?.name ?? "",
            ...
            <div className="flex-1 flex overflow-hidden gap-2 p-2 pt-1" data-monitor-layout>
              ...
              <div className="flex-1 flex flex-col min-w-0 gap-1.5">
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-1 h-5 rounded bg-cyan-500/8 border border-cyan-500/20 text-cyan-400/70 hover:text-status-info-fg hover:border-cyan-500/40 hover:bg-cyan-500/12 transition-colors shrink-0"
                >
              ...
              <button
                ...
                className="flex-shrink-0 flex items-center justify-center w-5 rounded bg-cyan-500/8 border border-cyan-500/20 text-cyan-400/70 hover:text-status-info-fg hover:border-cyan-500/40 hover:bg-cyan-500/12 transition-colors"
