    ...
    <div className="flex flex-col gap-3 px-4 py-4 pb-24">
      ...
      {installations.length > 1 && (
        <select
          ...
          className="w-full bg-muted border border-border rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:ring-1 focus:ring-blue-500"
        >
  ...
  return (
    <div className="rounded-xl border border-border bg-card opai-glass-soft-m overflow-hidden">
      <div className="px-4 py-2.5 border-b border-border flex items-center justify-between">
        <span className="text-sm font-medium text-foreground">{group.installationName}</span>
        <span className="text-xs text-muted-foreground">{group.guards.length} guardias</span>
      </div>
...
function GuardRow({ guard }: { guard: Guard }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3">
      <div className="flex-shrink-0 w-9 h-9 rounded-full bg-muted flex items-center justify-center text-xs font-semibold text-muted-foreground">
        {initials}
      </div>
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-2 mt-0.5">
          <span className="text-[10px] text-muted-foreground truncate">{guard.puestoName}</span>
          {guard.rut && (
            <>
