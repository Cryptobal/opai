...
export function PortalSupervisorNav({ active, onChange, onMoreOpen, visitasPendientes = 0 }: Props) {
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-card opai-glass-strong-m border-t border-border">
      <div className="flex items-center justify-around px-2 py-1 safe-area-inset-bottom">
        ...
        <button
          onClick={() => onMoreOpen?.()}
          className="relative flex flex-col items-center gap-0.5 py-2 px-3 min-w-[60px] rounded-lg transition-colors text-muted-foreground hover:text-foreground"
        >
