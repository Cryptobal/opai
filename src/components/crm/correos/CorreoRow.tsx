...
export function CorreoRow({ thread, onOpen }: { thread: CorreoThreadDTO; onOpen: () => void }) {
  return (
    <button
      type="button"
      onClick={onOpen}
      className="flex w-full flex-col gap-1.5 px-3 py-3 text-left ds-tap hover:bg-ds-surface-2"
    >
      ...
      <p className="truncate text-[13px] text-ds-text-2">{thread.subject || "(sin asunto)"}</p>
      {thread.snippet && (
