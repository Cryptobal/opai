      ...
      <CardContent className="pt-4 space-y-4">
        ...
        {hasAlerts && (
          <Link href="/ops/inventario/stock" className="block">
            <div className="rounded-lg border border-status-danger-border bg-status-danger-soft p-3 transition-colors hover:bg-ds-surface-2/20">
              <div className="flex items-center gap-2 mb-1.5">
                <AlertTriangle className="h-4 w-4 text-status-danger-fg" />
                <span className="text-sm font-semibold text-status-danger-fg">
                  Alertas de stock
                </span>
              </div>
              <div className="flex gap-3 flex-wrap">
                {stockData.outOfStockCount > 0 && (
                  <span className="text-xs bg-status-danger-soft text-status-danger-fg px-2 py-0.5 rounded-full font-medium">
                    {stockData.outOfStockCount} agotado{stockData.outOfStockCount > 1 ? "s" : ""}
                  </span>
                )}
                {stockData.belowMinimumCount > 0 && (
                  <span className="text-xs bg-status-warn-soft text-status-warn-fg px-2 py-0.5 rounded-full font-medium">
