...
export function OfflineBanner({ isOnline, pendingCount }: OfflineBannerProps) {
  ...
  if (showSyncing && isOnline) {
    return (
      <div className="fixed inset-x-0 bottom-[calc(56px+env(safe-area-inset-bottom))] z-40 flex items-center justify-center gap-2 bg-status-ok px-4 py-2 text-sm font-medium text-white backdrop-blur-sm transition-all">
  ...
  if (!isOnline) {
    return (
      <div className="fixed inset-x-0 bottom-[calc(56px+env(safe-area-inset-bottom))] z-40 flex items-center justify-center gap-2 bg-status-warn px-4 py-2 text-sm font-medium text-white backdrop-blur-sm transition-all">
