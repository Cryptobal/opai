...
import { Users, Eye, LogIn, Badge as BadgeIcon } from 'lucide-react';
  ...
  if (users.length === 0) {
    return (
      <div className="rounded-[10px] border border-border bg-card p-4 text-center">
        ...
        <p className="text-xs text-muted-foreground">Sin actividad en el portal (30d)</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-[10px] border border-border bg-card overflow-hidden">
      <div className="px-3.5 py-2.5 border-b border-border/50 flex items-center gap-2">
        ...
        <span className="text-xs font-bold">Top clientes en portal</span>
      ...
      <div className="divide-y divide-border/50">
        {users.map((user, i) => (
          <div key={`${user.accountId}-${i}`} className="px-3.5 py-2.5 flex items-center gap-2.5">
            <span className={`text-[11px] tabular-nums font-medium w-4 shrink-0 ${i < 3 ? 'text-teal-400' : 'text-muted-foreground'}`}>
              {i + 1}
            </span>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                <span className="text-[12px] font-medium truncate">{user.accountName}</span>
                {user.isProspect && (
            ...
            <div className="flex items-center gap-2.5 shrink-0 text-[10px] text-muted-foreground">
              <span className="flex items-center gap-0.5" title="Ingresos al portal">
                <LogIn className="h-3 w-3" /> {user.totalLogins}
              </span>
              <span className="flex items-center gap-0.5" title="Cotizaciones vistas">
                <Eye className="h-3 w-3" /> {user.totalQuoteViews}
              </span>
            </div>
          </div>
        ))}
