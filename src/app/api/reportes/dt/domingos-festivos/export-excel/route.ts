...
export async function POST(request: NextRequest) {
  try {
    ...
    wb.creator = "OPAI";
    const ws = wb.addWorksheet("Domingos y Festivos");
    ...
    for (const r of filtered) {
      ...
      ws.addRow({
        ...
        dia: tipoDia(esDomingo, esFeriado),
        ...
        horas: r.workedMinutes ? Math.round((r.workedMinutes / 60) * 100) / 100 : "",
      });
    }
    ...
    return new NextResponse(buffer, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="domingos-festivos-${from}-${to}.xlsx"`,
      },
    });
  } catch (error) {
    console.error("[DT] Error export-excel domingos-festivos:", error);
