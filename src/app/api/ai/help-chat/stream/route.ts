...
J. EMITIR BORRADOR AL SII: search_invoice_drafts → preview_emit_invoice_draft({draftId}) → OK del usuario → emit_invoice_draft({draftId, previewToken, confirm:true}). Mismo path que la UI (timbre + email PDF/XML). PROHIBIDO abrir /finanzas/facturacion/emitir ni emitir un borrador que el usuario no nombró.`
    : ""
}`
    : null;
