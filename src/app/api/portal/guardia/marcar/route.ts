/**
 * POST /api/portal/guardia/marcar
 * Registra marcación desde el portal del guardia (celular personal).
 * El guardia ya está autenticado (RUT+PIN al ingresar al portal).
 *
 * GET  → Devuelve el próximo tipo de marcación (entrada o salida).
 * POST → Registra la marcación.
 *
 * Cumplimiento Resolución Exenta N°38:
 * - GPS como evidencia, NUNCA bloquea la marcación
 * - Hash SHA-256 de integridad
 * - Sello de tiempo del servidor
 * - Comprobante electrónico al trabajador
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("[Portal Guardia] Marcar GET error:", error);
    return NextResponse.json({ success: false, error: "Error interno" }, { status: 500 });
  }
}

// ── POST: Register marcación ──
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!asignacion?.installation) {
      return NextResponse.json(
        { success: false, error: "No tienes una instalación asignada actualmente." },
        { status: 400 }
      );
    }
    ...
    // ── GEOLOCALIZACIÓN COMO EVIDENCIA (Res. Exenta N°38) ──
    // GPS es EVIDENCIA, nunca restricción. No se bloquea la marcación por ubicación.
    ...
    const result = await prisma.$transaction(async (tx) => {
      const marcacion = await tx.opsMarcacion.create({
        data: {
          ...
          hashIntegridad,
          atrasoMinutos,
          // Resolución Exenta N°38 — mandatory fields
    ...
    if (marcacionConfig.emailComprobanteDigitalEnabled && guardiaEmail) {
      sendMarcacionComprobante({
        ...
        lat: lat ?? null,
        lng: lng ?? null,
      }).catch((err) => console.error("[portal-guardia/marcar] Error enviando comprobante:", err));
    }
    ...
    if (gpsStatus === "fuera_rango") {
      import("@/lib/marcacion-email").then(({ sendNotificacionFueraDeRango }) =>
        ...
        }).catch((err) => console.error("[portal-guardia/marcar] Error notificando fuera de rango:", err))
      );
    }
    ...
    console.error("[portal-guardia/marcar] Error:", error);
    return NextResponse.json(
