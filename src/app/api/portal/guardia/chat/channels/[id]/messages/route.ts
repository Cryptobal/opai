/**
 * API Route: /api/portal/guardia/chat/channels/[id]/messages
 * GET  — List messages with cursor-based pagination for infinite scroll.
 * POST — Send a new message as guard.
 */
...
export async function GET(
  ...
  try {
    ...
    console.error("[Portal Guardia] Error listing chat messages:", err);
    return NextResponse.json(
    ...
    const message = await prisma.$transaction(async (tx) => {
      const msg = await tx.chatMessage.create({
        data: {
          ...
          channelId,
          senderType: "GUARD" as ChatSenderType,
          senderGuardiaId: session.guardiaId,
