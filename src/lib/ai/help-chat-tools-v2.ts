  ...
  return [
    {
      ...
      function: {
        ...
        description:
          "Lista o edita TODOS los costos adicionales de una cotización: costos extra (kind='cost': equipamiento, tecnología, movilización), uniformes (kind='uniform'), exámenes (kind='exam'), alimentación (kind='meal'), vehículos (kind='vehicle') e infraestructura (kind='infrastructure'). Úsala para 'agrega un adicional', 'suma cámaras por $X', 'incluye colación 30 días', 'agrega una camioneta con 40 km/día', 'suma una garita en arriendo', 'cambia el precio del uniforme', 'oculta la línea de movilización'. action=list (sin kind) devuelve TODOS los grupos: hazlo SIEMPRE antes y después de editar. Cada cambio recalcula el total mensual con el motor de costeo.",
        parameters: {
          type: "object",
          properties: {
            ...
            kind: {
              ...
              description: "Tipo de línea. OBLIGATORIO salvo action=list (list sin kind devuelve todos los grupos).",
            },
            ...
            quantity: { type: "number", description: "Cantidad. En kind=cost es la cantidad del ítem; en kind=infrastructure es el número de unidades." },
            unitPrice: { type: "number", description: "Precio unitario CLP (add/update en cost/uniform/exam)." },
            recurring: { type: "boolean", description: "Solo kind=cost. true = costo mensual recurrente (default); false = inversión one-time amortizada." },
            calcMode: { type: "string", enum: ["per_month", "per_guard"], description: "Solo kind=cost. per_month = monto fijo mensual (default); per_guard = el monto se multiplica por la dotación." },
            isEnabled: { type: "boolean", description: "Activa/desactiva la línea sin borrarla (cost/meal/vehicle/infrastructure). Deshabilitada no suma al costeo." },
            visibility: { type: "string", enum: ["visible", "hidden"], description: "visible (default) o hidden: oculta la línea en la propuesta al cliente sin sacarla del costo." },
            notes: { type: "string", description: "Solo kind=cost. Nota interna de la línea." },
            investmentAmount: { type: "number", description: "Solo kind=cost amortizable: monto total de la inversión (CLP). Obligatorio > 0 si recurring=false." },
            amortizationMonths: { type: "number", description: "Solo kind=cost amortizable: meses de amortización (1–120). Si se omite/nulo se usa la duración del contrato." },
            mealType: { type: "string", description: "kind=meal: tipo de comida (ej. 'Almuerzo', 'Cena'). Debe coincidir con un ítem de catálogo tipo comida para tomar su precio, o pasar priceOverride." },
            mealsPerDay: { type: "number", description: "kind=meal: comidas por día (0–10)." },
            daysOfService: { type: "number", description: "kind=meal: días de servicio al mes (0–31)." },
            priceOverride: { type: "number", description: "kind=meal: precio unitario CLP que reemplaza al del catálogo." },
            vehiclesCount: { type: "number", description: "kind=vehicle: cantidad de vehículos (≥1)." },
            rentMonthly: { type: "number", description: "kind=vehicle/infrastructure: arriendo mensual CLP." },
            kmPerDay: { type: "number", description: "kind=vehicle: kilómetros por día." },
            daysPerMonth: { type: "number", description: "kind=vehicle: días de uso al mes (0–31)." },
            kmPerLiter: { type: "number", description: "kind=vehicle: rendimiento km/litro (0 = sin cálculo de combustible)." },
            fuelPrice: { type: "number", description: "kind=vehicle/infrastructure: precio del litro de combustible CLP." },
            maintenanceMonthly: { type: "number", description: "kind=vehicle: mantención mensual CLP." },
            itemType: { type: "string", description: "kind=infrastructure: tipo de ítem (ej. 'Garita', 'Generador')." },
            hasFuel: { type: "boolean", description: "kind=infrastructure: true si consume combustible (habilita fuelLitersPerHour/fuelHoursPerDay/fuelDaysPerMonth/fuelPrice)." },
            fuelLitersPerHour: { type: "number", description: "kind=infrastructure: litros por hora." },
            fuelHoursPerDay: { type: "number", description: "kind=infrastructure: horas de uso al día (0–24)." },
            fuelDaysPerMonth: { type: "number", description: "kind=infrastructure: días de uso al mes (0–31)." },
          },
  ...
  try {
    await prisma.crmTask.createMany({
      data: items.map((it) => ({
        ...
        status: "open",
        type: "checklist",
        // dueAt ya viene como Date | null (mediodía UTC si era YYYY-MM-DD).
