/**
 * Dataset oficial de regiones y comunas de Chile (SUBDERE / INE).
 * Las claves del mapa coinciden 1:1 con el array REGIONES usado en el ATS.
...
/** Devuelve las comunas de una región. Array vacío si la región no existe. */
export function getComunasByRegion(region: string): string[] {
  return COMUNAS_POR_REGION[region] ?? [];
}

/** Lista plana de todas las comunas (útil para autocompletados globales). */
export function getAllComunas(): string[] {
  return Object.values(COMUNAS_POR_REGION).flat().sort();
}
