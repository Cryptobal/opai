...
/**
 * Build entity data for a contract document.
 */
export function buildContractEntityData(contract: {
  title: string;
  ...
  return {
    ...
    effectiveDate: contract.effectiveDate ? formatDateOnly(contract.effectiveDate) : null,
    expirationDate: contract.expirationDate ? formatDateOnly(contract.expirationDate) : null,
    durationMonths: contract.durationMonths ?? null,
  };
}

/**
 * Build enriched entity data for a CPQ quote.
 * Returns null if quote not found.
 */
export async function buildQuoteEnrichedData(quoteId: string): Promise<Record<string, any> | null> {
  const { prisma } = await import("@/lib/prisma");
  ...
  return {
    ...
    validUntil: quote.validUntil ? formatDateOnly(quote.validUntil.toISOString()) : null,
    totalPositions: quote.totalPositions,
    totalGuards: quote.totalGuards,
    monthlyCost: Number(quote.monthlyCost),
