/**
 * Motor de creación lead → cuenta + contacto + negocio + cotización(es).
 *
 * EXTRAÍDO MECÁNICAMENTE de `POST /api/crm/leads/[id]/approve` (Fase 15, B2) sin
 * cambios de comportamiento: el route sigue siendo la fuente HTTP y llama a este
 * servicio con los MISMOS inputs; el cockpit de Slack lo llama directo. Los
 * `return NextResponse.json(...)` originales se transformaron en un resultado
 * discriminado (`ApproveLeadResult`) que el route mapea byte-a-byte a su
 * respuesta previa. Cero cambios de precio/estructura de la cotización.
 */
...
export interface ApproveLeadDuplicate { id: string; name: string; rut: string | null; type: string }
export interface ApproveLeadExistingContact { id: string; firstName: string | null; lastName: string | null; email: string | null }
export type ApproveLeadResult =
  | { kind: "not_found" }
  | { kind: "already_approved" }
  | { kind: "no_pipeline" }
  | { kind: "duplicates"; duplicates: ApproveLeadDuplicate[]; existingContact: ApproveLeadExistingContact | null; installationConflicts: { name: string; id: string }[] | null; message?: string }
  | { kind: "conflict_installation"; installationName?: string; existingId?: string }
  | { kind: "bad_request"; error: string }
  | { kind: "tx_aborted"; error: string }
  | { kind: "error"; error: string }
  | { kind: "ok"; data: { account: unknown; contact: unknown; deal: unknown; quotes: Array<{ id: string; code: string; installationName?: string | null }> } };

export async function approveLeadToEntities(input: {
  tenantId: string;
  userId: string;
  leadId: string;
  body: any;
  ...
  try {
    ...
    if (err?.message === "CONFLICT_INSTALLATION") {
      return { kind: "conflict_installation", installationName: err.conflictName, existingId: err.existingId };
    }
    const msg = err?.message ?? "";
    if (msg.includes("La instalación seleccionada no existe")) {
      return { kind: "bad_request", error: msg };
    }
    ...
    return { kind: "error", error: msg || "Error al aprobar el lead" };
  }
}
