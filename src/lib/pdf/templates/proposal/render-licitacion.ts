  ...
  function EconomicTable() {
    return h(View, null,
      ...
      ...(opening.salariesByRole ?? []).length > 0
        ? [
            h(Text, { key: 'salaries-title', style: { ...styles.h1, fontSize: 10, marginTop: 12 } },
              'Sueldos por cargo · componentes del snapshot (por persona/mes)'),
            ...(opening.salariesByRole ?? []).flatMap((salary, salaryIndex) => [
              h(Text, {
                key: `salary-cargo-${salaryIndex}`,
                style: { fontSize: 8, fontWeight: 600, marginTop: salaryIndex === 0 ? 4 : 10 },
              }, `${salary.cargo} · ${salary.count.toLocaleString('es-CL')} persona(s)`),
              h(View, { key: `salary-head-${salaryIndex}`, style: styles.tableHeader },
                h(Text, { style: { ...styles.tableHeaderTxt, width: '62%' } }, 'Componente'),
                h(Text, { style: { ...styles.tableHeaderTxt, width: '38%', textAlign: 'right' } }, 'Monto'),
              ),
              ...salary.components.map((component, index) =>
                h(View, { key: `salary-${salaryIndex}-${index}`, style: styles.tableRow },
                  h(Text, { style: { fontSize: 7.5, width: '62%' } }, component.label),
                  h(Text, { style: { fontSize: 7.5, width: '38%', textAlign: 'right' } }, formatOpeningClp(component.amountClp)),
                ),
              ),
              h(View, { key: `salary-total-${salaryIndex}`, style: styles.ecoHighlight },
                h(Text, { style: { fontSize: 7.5, width: '62%', fontWeight: 700 } }, 'Costo empresa'),
                h(Text, { style: { fontSize: 7.5, width: '38%', textAlign: 'right', fontWeight: 700 } },
                  formatOpeningClp(salary.costoEmpresaClp)),
              ),
            ]),
          ]
        : [],
      h(Text, { key: 'direct-title', style: { ...styles.h1, fontSize: 10, marginTop: 12 } }, 'Costos directos'),
      ...(opening.directLines ?? []).length === 0
        ? [h(Text, { key: 'direct-empty', style: { fontSize: 8, color: '#64748b' } }, 'No aplica')]
        : [
            h(View, { key: 'direct-head', style: styles.tableHeader },
              h(Text, { style: { ...styles.tableHeaderTxt, width: '54%' } }, 'Concepto'),
              h(Text, { style: { ...styles.tableHeaderTxt, width: '14%', textAlign: 'right' } }, 'Cant.'),
              h(Text, { style: { ...styles.tableHeaderTxt, width: '32%', textAlign: 'right' } }, 'Monto mensual'),
            ),
            ...(opening.directLines ?? []).map((line, index) =>
              h(View, { key: `direct-${index}`, style: styles.tableRow },
                h(Text, { style: { fontSize: 8, width: '54%' } }, line.label),
                h(Text, { style: { fontSize: 8, width: '14%', textAlign: 'right' } },
                  line.quantity != null ? line.quantity.toLocaleString('es-CL') : '—'),
                h(Text, { style: { fontSize: 8, width: '32%', textAlign: 'right', fontWeight: 600 } },
                  formatOpeningClp(line.amountClp)),
              ),
            ),
          ],
      h(Text, { key: 'indirect-title', style: { ...styles.h1, fontSize: 10, marginTop: 12 } }, 'Costos indirectos'),
      ...(opening.indirectLines ?? []).length === 0
        ? [h(Text, { key: 'indirect-empty', style: { fontSize: 8, color: '#64748b' } }, 'No aplica')]
        : [
            h(View, { key: 'indirect-head', style: styles.tableHeader },
              h(Text, { style: { ...styles.tableHeaderTxt, width: '54%' } }, 'Concepto'),
              h(Text, { style: { ...styles.tableHeaderTxt, width: '14%', textAlign: 'right' } }, 'Cant.'),
              h(Text, { style: { ...styles.tableHeaderTxt, width: '32%', textAlign: 'right' } }, 'Monto mensual'),
            ),
            ...(opening.indirectLines ?? []).map((line, index) =>
              h(View, { key: `indirect-${index}`, style: styles.tableRow },
                h(Text, { style: { fontSize: 8, width: '54%' } }, line.label),
                h(Text, { style: { fontSize: 8, width: '14%', textAlign: 'right' } },
                  line.quantity != null ? line.quantity.toLocaleString('es-CL') : '—'),
                h(Text, { style: { fontSize: 8, width: '32%', textAlign: 'right', fontWeight: 600 } },
