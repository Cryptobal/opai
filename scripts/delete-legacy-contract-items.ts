/**
 * One-shot: borra definitivamente los FinanceCashflowItem con source=CONTRACT
 * que quedaron en DB tras la migración a contratos 100% manuales (2026-05-11).
 * Los contratos ahora se agregan desde el tab "Contratos" del CRM con
 * source=OTHER y sourceRefId=Document.id.
 *
 * Cascade: FinanceCashflowOccurrence y FinanceCashflowIpcAdjustment caen
 * por onDelete: Cascade en el schema.
 *
 * Uso:
 *   TENANT_SLUG=gard npx tsx scripts/delete-legacy-contract-items.ts
...
async function deleteTenant(tenantId: string, label: string) {
  ...
  for (const item of items) {
    ...
    console.log(
      `  · ${item.name} (${Number(item.amount)} ${item.currency})${flag}`,
    );
  }
...
async function main() {
  ...
  if (tenants.length === 0) {
    console.error(
      `❌ No se encontró ningún tenant${slug ? ` con slug "${slug}"` : ""}`,
    );
    process.exit(1);
  }

  console.log(
    `🚀 Borrando legacy contract items en ${tenants.length} tenant(s)…\n`,
  );
