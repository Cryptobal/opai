...
import { seedCpqFixedSections } from './seeds/cpq-fixed-sections';
...
async function main() {
  ...
  for (const setting of settings) {
    const existing = await prisma.setting.findFirst({ where: { key: setting.key, tenantId: null }, select: { id: true } });
    if (existing) {
  ...
  // 5. Seed CPQ data
  await seedCpqData();
  // 5b. Secciones fijas institucionales (propuesta v3)
  await seedCpqFixedSections(prisma, tenant.id);
  // 6. Seed CRM data
