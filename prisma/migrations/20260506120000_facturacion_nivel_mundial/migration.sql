-- ════════════════════════════════════════════════════════════════════
-- Facturación Nivel Mundial: DRAFT, UF, recurrentes, email log, XML config
-- ════════════════════════════════════════════════════════════════════
...
ALTER TABLE "finance"."finance_dtes"
  ...
  ADD COLUMN "uf_date_at_issue"  DATE;

-- ── 3. FinanceDteLine: precio original UF ────────────────────────────
ALTER TABLE "finance"."finance_dte_lines"
  ADD COLUMN "unit_price_uf" DECIMAL(14,4);

-- ── 4. TenantDteConfig: emails XML al backoffice + plantilla email ──
ALTER TABLE "public"."tenant_dte_configs"
  ADD COLUMN "default_xml_recipient_emails"      TEXT[] NOT NULL DEFAULT ARRAY[]::TEXT[],
  ADD COLUMN "default_xml_recipient_always_send" BOOLEAN NOT NULL DEFAULT FALSE,
  ADD COLUMN "email_template_subject"            TEXT,
  ADD COLUMN "email_template_body"               TEXT;

-- ── 5. FinanceDteRecurringTemplate ───────────────────────────────────
CREATE TABLE "finance"."finance_dte_recurring_templates" (
...
CREATE INDEX "idx_dte_recurring_run_template"
  ON "finance"."finance_dte_recurring_runs" ("tenant_id", "template_id");

-- ── 7. FinanceDteEmailLog ────────────────────────────────────────────
CREATE TABLE "finance"."finance_dte_email_logs" (
  "id"            UUID NOT NULL DEFAULT uuid_generate_v4(),
  "tenant_id"     TEXT NOT NULL,
...
CREATE INDEX "idx_dte_email_log_dte"
  ON "finance"."finance_dte_email_logs" ("tenant_id", "dte_id");
CREATE INDEX "idx_dte_email_log_sent"
